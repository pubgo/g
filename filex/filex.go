...
func MustOpen(fileName, filePath string) (f *os.File, err error) {
	...
	dir, err := os.Getwd()
	xerror.Panic(err, "os.Getwd Error")

	src := filepath.Join(dir, filePath)
	xerror.Assert(CheckPermission(src), "file.CheckPermission Permission denied src: %s", src)
	xerror.PanicF(IsNotExistMkDir(src), "file.IsNotExistMkDir src: %s", src)
...
func AppendFile(filename string, data string) {
	...
	if f, err := os.OpenFile(filename, os.O_WRONLY, 0644); err != nil {
		xerror.Panic(err, "os.OpenFile错误")
	} else {
		if n, err := f.Seek(0, 2); err != nil {
			xerror.Panic(err, "f.Seek错误")
		} else {
			if _, err = f.WriteAt([]byte(data), n); err != nil {
				xerror.Panic(err, "f.WriteAt错误")
			}
		}
		_ = f.Close()
	}

...
func InitUser() {
	...
	BaseRoutes.Users.Handle("/valid", ApiHandler(valid)).Methods("GET")
...
func valid(c *Context, w http.ResponseWriter, r *http.Request) {
	claims, err := app.ValidateTokenMiddleware(w, r)
	if err != nil {
		w.WriteHeader(http.StatusUnauthorized)
	} else {
		buff, _ := json.Marshal(claims)
...
func signupByMobile(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	user := &model.User{Username: username, Nickname: nickname, Mobile: mobile, Password: password, AllowMarketing: true, Roles: roles}
	if user.Roles == "teacher" {
		user.ReviewResult = "pending"
	} else {
		user.ReviewResult = "pass"
	}
